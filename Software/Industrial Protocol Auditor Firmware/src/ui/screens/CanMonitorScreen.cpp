...
void CanMonitorScreen::buildUiStats()
{
    ...
    // Bus load bar.
    lv_obj_t* label_load = lv_label_create(_stats_view);
    lv_label_set_text(label_load, "BUS LOAD:");
    lv_obj_align(label_load, LV_ALIGN_TOP_LEFT, 5, 5);
    lv_obj_set_style_text_color(label_load, lv_color_white(), 0);
    lv_obj_set_style_text_font(label_load, &lv_font_unscii_8, 0);
    ...
    lv_obj_set_style_text_font(_label_error_count, &lv_font_unscii_8, 0);
    ...
    lv_obj_set_style_text_font(_label_unique_ids, &lv_font_unscii_8, 0);
...
void CanMonitorScreen::updateSniffer()
{
    ...
    int data = esp_random() % 256;
    // Give it format for the display.
    char buffer[32];
    sprintf(buffer, "ID:%03X [%02X...]\n", id, data);
    // Add to the terminal.
...
void CanMonitorScreen::toggleMode()
{
    ...
    if (_current_mode == MODE_SNIFFER)
    {
        ...
        lv_label_set_text(_header_label, "DASHBOARD");
        lv_obj_set_style_text_font(_label_error_count, &lv_font_montserrat_10, 0);
    } else {
        _current_mode = MODE_SNIFFER;
        lv_obj_add_flag(_stats_view, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(_sniffer_view, LV_OBJ_FLAG_HIDDEN);
        lv_label_set_text(_header_label, "SNIFFER LOG");
        lv_obj_set_style_text_font(_label_error_count, &lv_font_montserrat_10, 0);
    }
